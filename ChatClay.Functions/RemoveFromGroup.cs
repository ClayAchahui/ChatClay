...
namespace ChatClay.Functions
{
	public static class RemoveFromGroup
	{
		...
		public static async Task Run(
				...
				ILogger log)
		{
			var message = new JsonSerializer().Deserialize<UserConnectedMessage>(new JsonTextReader(new StreamReader(req.Body)));
			...
			TableQuery<UserEntity> rangeQuery = new TableQuery<UserEntity>().Where(TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, message.Sender));
